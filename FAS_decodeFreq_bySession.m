...
onlyEXC = true; % only use EXC neurons in sessions
lateRespWin = true; % use the later response window (3-4 sec post-stim)
...
for ns = 1:length(sessIDs) %nt = 1 : length(selType)
    ...
    if multiAmp == true
        locPVs = find(is.sessID(:,ns) & is.PV & is.allMod); 
        ...
        % loc neurons from this mouse (but not this session)
        otherPVs = find(locIdx & is.PV & is.allMod);
