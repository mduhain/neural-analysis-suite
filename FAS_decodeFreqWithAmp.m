...
mappedAmp = [20;35;70;45;30]; % amplitude in micrometers for [300, 500, 700, 900, 1100 hz]
...
for i = 1:size(Tc,1)
    ...
    if multiAmp %DATASET2
        ...
        if  minTrialCountsF(i) >= minTrialNum
            ...
            [~,locF2] = ismember(cellFreq(locF>1),freqVal); % find loc vals from freqVal
            Tc.amplitudes{i} = mappedAmp(locF2); %assign matching amp values
        else
            Tc.frequencies{i} = [];
            Tc.responses{i} = [];
        end
...
Tc = movevars(Tc, "amplitudes", "Before", "sessID"); %fix order
...
ampVal = zscore(mappedAmp);

if multiAmp == true
    nCellDraws = 20:20:1000;
elseif multiAmp == false
    % nCellDraws = 15:15:480;
    nCellDraws = 480;
end
...
for nd = 1 : length(nCellDraws)
    ...
    trainAmp = repmat(ampVal,trainTrial,1);
    ...
    for ni = 1:length(cellType)
        ...
        for nj = 1:nIter
            ...
            if usePCA
                ...
                trainResp_comp = [trainResp_red, trainAmp];
                mdl = fitcdiscr(trainResp_red,trainFreq,'DiscrimType','linear');
                mdl2 = fitcdiscr(trainResp_comp,trainFreq,'DiscrimType','pseudoLinear');
...
set(gca,'XLim',[15 500],'XTick',[15 75 125 175 225 275 325 375 425 480],...
    'YLim',[30 80],'YTick',30:10:80);
