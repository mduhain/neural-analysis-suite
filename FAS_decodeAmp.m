...
nCellDraws = 22; % over-estimation to poulate arrays 60 (amp.sel vs. random) or 22 (4 sel types)
...
PCTall = zeros(nFreq,nIter,length(selType),nCellDraws,length(cellType)); % (na,nj,nt,nd,nc)
% Models (all)
% MDLall = cell(nIter,length(selType),length(nCellDraws),length(cellType)); % (nj,nt,nd,nc)
MDLall = cell(nFreq,nIter,length(selType),nCellDraws,length(cellType)); % (na,nj,nt,nd,nc)



for nf = 2 : 2 %nFreq %num freqs
...
cd('C:\Users\skich\Desktop\WORK')
load("C:\Users\skich\Desktop\WORK\modelAccuracy_dataset2_ampDecode_selVsRandom.mat")
