...
accMat = zeros(2,length(cellType)); % col.1 (acc mean, acc CI), col.2 (PV, SOM, EXC)
...
PCTloc = squeeze(mean(PCTallEXC_baseline(:,~trainedSess,1),1))' .* 100;
PCTloc(PCTloc == 0) = NaN;
...
for npt = 2 : 3 % num pair types (EXC:EXC, EXC:PV, EXC:SOM, PV:PV, SOM:SOM)
    ...
    % SAVE AVERAGE ACCURACIES
    accMat(1,npt-1) = mean(PCTloc,'omitnan');
    locCI = mkCI(PCTloc);
...
set(gca,'XLim',[0.2 3.8],'XTick',1:3,'XTickLabel',{'EXC&PV','EXC&SOM','EXC alone'});
